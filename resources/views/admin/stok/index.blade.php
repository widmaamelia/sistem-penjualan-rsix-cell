...
<div class="summary-cards">
    ...
    <div class="summary-card">
        ...
        <div class="summary-info">
            ...
            <h3 style="color: #ef4444;">{{ number_format($stokRendah, 0, ',', '.') }}</h3>
        </div>
    </div>
    <div class="summary-card">
        ...
        <div class="summary-info">
            <h6>Stok Habis</h6>
            <h3>{{ number_format($stokHabis, 0, ',', '.') }}</h3>
        </div>
    </div>
</div>
...
<div class="action-bar">
    <form action="{{ route('stok.index') }}" method="GET" class="filter-group m-0" id="filterForm">
        ...
        <select name="id_cabang" class="form-select filter-select" onchange="document.getElementById('filterForm').submit()">
            ...
            @foreach($cabangs as $cabang)
                <option value="{{ $cabang->id_cabang }}" {{ request('id_cabang') == $cabang->id_cabang ? 'selected' : '' }}>
                    {{ $cabang->nama_cabang }}
                </option>
            @endforeach
        </select>
...
<div class="table-container">
    <table class="table mb-0">
        ...
        <tbody>
            @php $no = ($produks->currentPage() - 1) * $produks->perPage() + 1; @endphp
            @forelse($produks as $produk)
                ...
                <tr>
                    ...
                    <td class="fw-bold" style="font-size: 12px;">{{ $produk->sku ?? '-' }}</td>
                    <td>
                        <div class="product-cell">
                            <img src="{{ $produk->foto_produk ?? 'https://via.placeholder.com/40' }}" alt="Foto" class="product-img">
                            <div>
                                <span class="fw-bold text-dark d-block">{{ Str::limit($produk->nama_produk, 30) }}</span>
                                @if($produk->kategori && $produk->kategori->nama_kategori == 'Pulsa & Paket Data')
                                    <small class="text-primary fw-bold" style="font-size: 10px;">{{ strtoupper(explode(' ', $produk->nama_produk)[1] ?? 'PROVIDER') }}</small>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="text-muted">{{ $produk->kategori->nama_kategori ?? '-' }}</td>
                    <td class="text-center fw-bold fs-6" style="{{ $totalSeluruhStok <= 0 ? 'color: #ef4444;' : 'color: #111827;' }}">{{ $totalSeluruhStok }}</td>
                    <td>
                        @if($status == 'aman')
                    ...
                    <td>
                        <div class="action-icons">
                            ...
                            <a href="{{ route('stok.history', $produk->id_produk) }}" title="Riwayat Stok"><i class="fa-solid fa-clock-rotate-left"></i></a>
                        </div>
                    </td>
                </tr>
            @empty
    ...
    <div class="bg-white border-top py-3 px-4 d-flex justify-content-between align-items-center" style="font-size: 13px; color: #6b7280;">
        <div>
            @if($produks->previousPageUrl())
                <a href="{{ $produks->previousPageUrl() }}" class="text-decoration-none text-muted"><i class="fa-solid fa-chevron-left me-1"></i> Sebelumnya</a>
            @else
        ...
        <div>
            @if($produks->nextPageUrl())
                <a href="{{ $produks->nextPageUrl() }}" class="text-decoration-none" style="color: #1a5ca6;">Berikutnya <i class="fa-solid fa-chevron-right ms-1"></i></a>
            @else
