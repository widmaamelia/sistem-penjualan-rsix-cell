...
<div class="action-bar">
    <form id="searchForm" action="{{ route('kategori.index') }}" method="GET" class="search-input m-0">
        <i class="fa-solid fa-magnifying-glass" style="z-index: 10;"></i>
        <input type="text" id="searchInput" name="search" class="form-control" placeholder="Cari kategori..." value="{{ request('search') }}" autofocus>
    </form>
    <div>
        <a href="{{ route('kategori.create') }}" class="btn btn-primary-custom text-decoration-none">
...
<div class="table-container">
    <table class="table mb-0">
        ...
        <tbody>
            @forelse($kategoris as $index => $kategori)
                <tr>
                    <td class="text-muted" style="padding-left: 20px;">{{ $kategoris->firstItem() + $index }}</td>
                    <td class="fw-medium text-dark">{{ $kategori->nama_kategori }}</td>
                    <td>
                    ...
                    <td>
                        <div class="action-icons">
                            <!-- Edit Button -->
                            <a href="{{ route('kategori.edit', $kategori->id_kategori) }}" class="text-primary" title="Edit">
                            ...
                            <button type="button" class="text-danger btn-delete-kategori" title="Hapus"
                                ...
                                data-produk="{{ $kategori->produks_count }}">
    ...
    <div class="bg-white border-top py-3 px-4 d-flex justify-content-between align-items-center" style="font-size: 13px; color: #6b7280;">
        @if($kategoris->count() > 0)
            <div>Menampilkan <strong>{{ $kategoris->firstItem() }}-{{ $kategoris->lastItem() }}</strong> dari <strong>{{ $kategoris->total() }}</strong> kategori</div>
            <div>
...
<script>
    ...
    function attachDeleteListeners() {
        document.querySelectorAll('.btn-delete-kategori').forEach(btn => {
            ...
            newBtn.addEventListener('click', function() {
                ...
                formDelete.action = "{{ url('/kategori') }}/" + id;
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            fetch("{{ route('kategori.index') }}?search=" + encodeURIComponent(query), {
